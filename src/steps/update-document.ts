...
export const UpdateDocumentStepTask = {
  ...
  inputSchema: [
    ...
    },
    {
      ...
      type: 'text',
      admin: {
        description: 'The ID of the document to update. Use whole-string JSONata templates to reference IDs (e.g., "{{trigger.doc.id}}" or "{{steps.previousStep.output.id}}")'
      },
      required: true
    },
    {
      ...
      type: 'json',
      admin: {
        description: 'The data to update the document with. Use whole-string JSONata templates to reference values (e.g., {"status": "{{trigger.doc.status}}"})'
      },
      required: true
    },
    {
